...
void renderSurface(SDL_Surface *surface) {
  ...
  SDL_Window *window = SDL_CreateWindow(
      "SDL Surface Renderer", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
...
GLuint LoadCubeMap(const std::vector<std::filesystem::path> &faces) {
  ...
  for (unsigned int i = 0; i < faces.size(); i++) {
    TextureFromFile(textureID, faces[i], GL_TEXTURE_CUBE_MAP,
                    GL_TEXTURE_CUBE_MAP_POSITIVE_X + i);
  }
  SetupTextureSampling(GL_TEXTURE_CUBE_MAP, textureID, false);
...
void TextureFromFile(const GLuint tex, const std::filesystem::path &fileName,
  ...
  glTexImage2D(
      Role,    // melyik binding point-on van a textúra erőforrás, amihez
               // tárolást rendelünk
      ...
      GL_UNSIGNED_BYTE,       // forrás egy pixelének egy csatornáját hogyan
                              // tároljuk
...
void SetupTextureSampling(GLenum Target, GLuint textureID,
  ...
  if (generateMipMap)
    glGenerateMipmap(Target); // Mipmap generálása
  glTexParameteri(
